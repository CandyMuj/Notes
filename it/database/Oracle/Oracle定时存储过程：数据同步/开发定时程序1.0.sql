...
comment on column timingrun_config.exec_cycle is '执行周期，单位(天)';
...
BEGIN
  ...
  DBMS_JOB.SUBMIT(JOB       => job,  -- 自动生成的任务id
                  ...
                  NEXT_DATE => sysdate + exec_cyclea/(24 * 60 * 60), -- 初次执行时间,立刻执行
                  INTERVAL  => 'sysdate + '||exec_cyclea||'/(24 * 60 * 60)');  -- 执行周期
  COMMIT;
